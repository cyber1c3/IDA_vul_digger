...
def read_check_32():
    for functionAddr in idautils.Functions():# 检查所有函数
        if "read" in idc.GetFunctionName(functionAddr):
            ...
            for xref in xrefs:# 检查交叉引用是否是函数调用   
                ...
                if idc.GetMnem(xref).lower() == "call":  
                    ...
                    if "offset" in opnd:
                        pre_addr=xref
                        while True:
                            pre_addr=idc.PrevHead(pre_addr)
                            pre_op=idc.GetOpnd(pre_addr,0)
                            # print hex(pre_addr),pre_op
                            if "offset" in pre_op:
                                var_addr=idc.GetOperandValue(pre_addr,0)
                    ...
                    _addr = xref
                    while True:
                        _addr = idc.PrevHead(_addr)#获取call指令的上一条指令地址
                        ...
                        elif _op in ("mov","lea") and idc.GetOpnd(_addr, 0) == opnd and "offset" in idc.GetOpnd(_addr, 1):
                            var_addr=idc.GetOperandValue(pre_addr,1)
                            var_size =idc.get_item_size(var_addr)
                            if size>var_size:
                                print "[*] find a bss overflow call at:0x%x,buf: 0x%x[size:0x%x],input size:0x%x"%(xref,var_addr,var_size,size)
                            else:
