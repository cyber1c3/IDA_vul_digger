...
def get_printf_plt():
    ...
    for func in idautils.Functions():                               # 获取当前程序所有的函数
        ...
        if '.printf' in idc.GetFunctionName(func) or 'printf' in idc.GetFunctionName(func):                  # 获取printf plt的地址                
            printf_plt = func
            return printf_plt
        if 'printf' in idc.GetFunctionName(func):
    ...
    if len(printf_func)>1:
        ...
        for x in printf_func:
            print "0x%x -> %s".format(x,idc.GetFunctionName(x))
...
def get_scanf_plt():
    ...
    for func in idautils.Functions():                               # 获取当前程序所有的函数
        # print hex(func), idc.GetFunctionName(func) 
        if 'isoc99_scanf' in idc.GetFunctionName(func) or 'isoc99_scanf' in idc.GetFunctionName(func):                  # 获取scanf的plt的地址               
            printf_plt = func
            return printf_plt
    ...
    for xref in printf_plt_xrefs:                                   # 遍历所有的call printf
        ...
        if bits == 64: 
            for i in range(0, number):
                ...
                if idc.GetMnem(now_addr) == 'mov':                      # 向前寻找到mov指令          
                    if idc.GetOpnd(now_addr, 0) in ['rdi','edi']:       # 判断arg0是否是rdi或edi
                        ...
                        if idc.GetOpType(now_addr, 1) == idaapi.o_reg:         # 检查操作数2类似是否是寄存器，如果是，可能存在格式化字符串漏洞。
            ...
            for i in range(0, number):
                ...
                if idc.GetMnem(now_addr) in ("jmp","call","ret","retn"):
                    break


                if (idc.GetMnem(now_addr) == 'push' ):
                    if  'offset' in idc.GetOpnd(now_addr, 0) :             # 白名单，push的是一个变量
                        break

                    elif idc.GetOpType(now_addr, 0) in [ idaapi.o_reg, idaapi.o_displ]:
                        # if 'ebp' in idc.GetOpnd(now_addr, 0):
                        #     break
                        print "[!] Might be a format string vulnerablity : 0x%x %s [WARNING!]" % (now_addr,\
                                     idc.GetDisasm(now_addr))
                        break
                if (idc.GetMnem(now_addr) == 'mov' )and ('esp' in idc.GetOpnd(now_addr, 0)):
                    if idc.GetOpType(now_addr, 1) in [ idaapi.o_reg, idaapi.o_displ ]:
                        print "[!] Might be a format string vulnerablity : 0x%x %s [WARNING!]" % (now_addr,\
                                     idc.GetDisasm(now_addr))
                    break
...
def find_scanf_fsb(scanf_plt, bits):      
    ...
    for xref in scanf_plt_xrefs:                                   # 遍历所有的call scanf
        ...
        if bits == 64: 
            ...
            for i in range(0, number):
                now_addr = idc.PrevHead(now_addr)
                if idc.GetMnem(now_addr) in ("jmp","call","ret","retn"):
                    break
